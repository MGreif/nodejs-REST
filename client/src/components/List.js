...
export default class List extends React.Component {
  constructor(props) {
    ...
    this.multiselect = {
      ...
      validAmount: 1,
      isValid: (curr) =>
        curr > this.multiselect.validAmount && this.multiselect.isActive,
    };
  ...
  deleteComp(elem) {
    return (
      <DialogComponent
        opener={<DeleteIcon fontSize="small" />}
    ...
    return {
      checkBoxes: (elem) => (
        <Checkbox
          ...
          onChange={(e) => {
            this.checkMultiselect(elem._id);
          }}
        />
      ),
      deleteButton: (
        <DialogComponent
          ...
          options={[
            { name: "Abbrechen" },
            {
              name: "Löschen",
              action: () => {
                this.state.multiselect.forEach((elem) =>
  ...
  render() {
    if (this.props.data.length === 0)
      return <div>No Data Found {this.props.add} </div>;
      ...
      <>
          ...
          <Table className={this.classes.table} aria-label="customized table">
            <TableHead>
              <TableRow>
                ...
                {Object.keys(this.props.data[0]).map((elem) => (
                ...
                <StyledTableRow key={elem._id}>
                  ...
                  {this.props.multiselect && (
                    <StyledTableCell align="center">
                      {this.multiselectBoxes().checkBoxes(elem)}
                    </StyledTableCell>
                  )}

                  {Object.values(elem).map((e) => (
                    <StyledTableCell>{e.toString()}</StyledTableCell>
                  ))}

                  {(this.props.delete || this.props.edit) && (
                    <StyledTableCell align="center">
                      <div className="flex-box row">
                        {this.props.delete && this.deleteComp(elem)}
        ...
        <div className="flex-box row">
          ...
          {this.multiselect.isValid(this.state.multiselect.length) &&
            this.multiselectBoxes().deleteButton}
        </div>
      </>
    );
  }
